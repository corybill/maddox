...
class FrameworkRouteScenario extends Scenario {
   ...
   *
   * @param {{ mockName: string, path: string, module: { default: unknown, loader?: Function, action?: Function, HydrateFallback?: Function, ErrorBoundary?: Function }, id?: string, children?: unknown[], HydrateFallback?: Function, ErrorBoundary?: Function }} descriptor
  ...
  addStub(descriptor) {
    ...
    this._routeDescriptors_.push({
      ...
      ErrorBoundary: descriptor.ErrorBoundary
    });
  ...
  /**
   * Add a middleware function that can transform the incoming Request object
   * before it reaches the route's loader or action. The middleware receives
   * the current Request and must return a new Request (or the same one)
   * which will be passed to the next middleware or the final route handler.
   *
   * @param {(request: Request) => Request | Promise<Request>} middlewareFn
   * @returns {FrameworkRouteScenario}
   */
  withRequestMiddleware(middlewareFn) {
    this._assertConfigBeforeRender_('withRequestMiddleware');

    preconditions
      .shouldBeFunction(middlewareFn, ErrorFactory.build(constants.errorMessages.FrameworkRouteMiddlewareCallback))
      .debug({ middlewareFn })
      .test();

    this._requestMiddleware_.push(middlewareFn);
    return this;
  }
  ...
  /**
   * Safely monkey-patches global.Request to handle JSDOM URLSearchParams compatibility.
  ...
  _applyRequestPatch_() {
    ...
    if (OriginalRequest) {
      global.Request = class extends OriginalRequest {
        constructor(input, init) {
          if (init && init.body && init.body.constructor.name === 'URLSearchParams') {
    ...
    // Ensure FormData is JSDOM's version if we are in JSDOM
    if (global.window && global.window.FormData) {
      global.FormData = global.window.FormData;
      if (typeof globalThis !== 'undefined') {
        globalThis.FormData = global.window.FormData;
      }
    }
  ...
  async _runStubTestBody_() {
    ...
    try {
      restoreRequest = this._applyRequestPatch_();
      ...
      const wrapRoutes = (routeDescriptors) => {
        return routeDescriptors.map((d) => {
          const route = { path: d.path, Component: d.module.default };
          const hydrateFallback = d.HydrateFallback ?? d.module.HydrateFallback;
          const errorBoundary = d.ErrorBoundary ?? d.module.ErrorBoundary;
          ...
          if (d.children) {
            route.children = wrapRoutes(d.children.map(child => ({
              ...child,
              module: child.module || { default: child.Component, loader: child.loader, action: child.action, ErrorBoundary: child.ErrorBoundary }
            })));
          }
      ...
      const routes = wrapRoutes(this._routeDescriptors_);
      ...
      if (typeof restoreRequest === 'function') {
        restoreRequest();
      }
  ...
  _wrapForCapture_(mockName, kind, originalFn, invocations) {
    ...
    return async function maddoxCaptureWrapper(args) {
      if (args && args.request && self._requestMiddleware_.length > 0) {
        for (const middleware of self._requestMiddleware_) {
          args.request = await Promise.resolve(middleware(args.request));
        }
      }
