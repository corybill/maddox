...
class HttpReqScenario extends Scenario {
  ...
  withHttpRequest(request) {
    preconditions
      .shouldBeArray(request, ErrorFactory.build(constants.errorMessages.HttpRequestArray))
  ...
  resShouldBeCalledWith(funcName, params) {
    preconditions
      .shouldBeString(funcName, ErrorFactory.build(constants.errorMessages.ResShouldBeCalledWithFunctionString))
  ...
  resShouldContainHeader(headerName, headerValue, funcName) {
    preconditions
      .shouldBeString(headerName, ErrorFactory.build(constants.errorMessages.HeaderNameShouldBeString))
      .debug({ headerName, headerValue })
      .test();
    preconditions
      .shouldBeString(headerValue, ErrorFactory.build(constants.errorMessages.HeaderValueShouldBeString))
      .debug({ headerName, headerValue })
      .test();
  ...
  resShouldAlwaysBeIgnored(funcName) {
    preconditions
      .shouldBeString(funcName, ErrorFactory.build(constants.errorMessages.ShouldBeCalledWithFunctionString))
      .debug({ mockName: constants.ResponseMockName, funcName })
      .test();
  ...
  _validateScenario_(testable) {
    ...
    preconditions
      .shouldBeDefined(entryPointFunction, ErrorFactory.build(constants.errorMessages.MissingEntryPoint))
      .debug({ entryPointFunction })
      .test();

    preconditions
      .shouldBeDefined(inputParams, ErrorFactory.build(constants.errorMessages.HttpReqUndefined))
      .debug({ inputParams })
      .test();

    preconditions
      .checkArgument(hasFinisherBeenSet, ErrorFactory.build(constants.errorMessages.ExactlyOneResponseFinisher))
      .debug({ responseFinishers: Object.keys(constants.ResponseEndFunctions) })
      .test();
  }
}

export default HttpReqScenario;
