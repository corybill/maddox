    ...
    this._testRunnable_ = () => {
      return new Promise((resolve, reject) => {
        try {
          ...
          preconditions.shouldBeFalsey((result instanceof BluebirdPromise), ErrorFactory.build(constants.errorMessages.ResponseCannotBePromise))
            .set("isPassThroughError", true).test();

          preconditions.shouldBeFalsey((result instanceof Promise), ErrorFactory.build(constants.errorMessages.ResponseCannotBePromise))
            .set("isPassThroughError", true).test();
