...
import ExecuteTest from '../steps/execute-test.js';
import WaitForTestFinisher from '../steps/wait-for-test-finisher.js';
import TestMocks from '../steps/test-mocks.js';
import ExecutePerf from '../steps/execute-perf.js';
import FinishTest from '../steps/finish-test.js';
import SkipTest from '../steps/skip-test.js';
import HandleError from '../steps/handle-error.js';
...
function normalizeCallbackDataToReturn(dataToReturn) {
  if (
    ...
    Object.keys(dataToReturn).length === 0
  ) {
    return [];
  }
...
class Scenario {
  constructor(testContext) {
    ...
    if (this._testContext_) {
      preconditions
        .shouldBeDefined(this._testContext_.skip, ErrorFactory.build(constants.errorMessages.IllegalMochaThisContext))
        .test();
  ...
  mockThisFunction(mockName, funcName, object) {
    preconditions
      .shouldBeString(mockName, ErrorFactory.build(constants.errorMessages.MockThisFunctionMockString))
      .debug({ mockName, funcName })
      .test();
    preconditions
    ...
    preconditions
      ...
      .debug({ mockName, funcName, object })
      .test();
  ...
  withEntryPoint(entryPointObject, entryPointFunction) {
    preconditions
      .shouldBeObject(entryPointObject, ErrorFactory.build(constants.errorMessages.EntryPointObject))
      .debug({ entryPointObject, entryPointFunction })
      .test();
    preconditions
    ...
    preconditions
      .shouldBeFunction(
        ...
        ErrorFactory.build(constants.errorMessages.EntryPointFunction)
      )
      .debug({ entryPointObject, entryPointFunction })
      .test();
  ...
  withInputParams(inputParamsIn) {
    preconditions
      .shouldBeArray(inputParamsIn, ErrorFactory.build(constants.errorMessages.InputParamsArray))
      .debug({ inputParamsIn: inputParamsIn })
      .test();
  ...
  shouldBeCalledWith(mockName, funcName, params) {
    preconditions
      .shouldBeString(mockName, ErrorFactory.build(constants.errorMessages.ShouldBeCalledWithKeyString))
      .debug({ mockName, funcName })
      .test();
    preconditions
    ...
    preconditions
      ...
      .debug({ mockName, funcName, params })
      .test();
  ...
  shouldBeCalled(mockName, funcName) {
    preconditions
      .shouldBeString(mockName, ErrorFactory.build(constants.errorMessages.ShouldBeCalledKeyString))
      .debug({ mockName, funcName })
      .test();
    preconditions
      .shouldBeString(funcName, ErrorFactory.build(constants.errorMessages.ShouldBeCalledFunctionString))
      .debug({ mockName, funcName })
      .test();
  ...
  doesReturn(mockName, funcName, dataToReturn) {
    preconditions
      .shouldBeString(mockName, ErrorFactory.build(constants.errorMessages.DoesReturnMockName))
      .debug({ mockName, funcName })
      .test();
    preconditions
      ...
      .test();
    preconditions
      .shouldBeDefined(dataToReturn, ErrorFactory.build(constants.errorMessages.DoesReturnDataToReturn))
      .debug({ mockName, funcName })
      .test();
  ...
  doesReturnWithPromise(mockName, funcName, dataToReturn) {
    preconditions
      .shouldBeString(mockName, ErrorFactory.build(constants.errorMessages.DoesReturnPromiseMockName))
      .debug({ mockName, funcName })
      .test();
    preconditions
      ...
      .test();
    preconditions
      .shouldBeDefined(dataToReturn, ErrorFactory.build(constants.errorMessages.DoesReturnPromiseDataToReturn))
      .debug({ mockName, funcName })
      .test();
  ...
  doesReturnWithCallback(mockName, funcName, dataToReturn) {
    ...
    preconditions
      .shouldBeString(mockName, ErrorFactory.build(constants.errorMessages.DoesReturnCallbackMockName))
      .debug({ mockName, funcName })
      .test();
    preconditions
      ...
      .test();
    preconditions
      .shouldBeArray(dataToReturn, ErrorFactory.build(constants.errorMessages.DoesReturnCallbackDataToReturn))
      .debug({ mockName, funcName, dataToReturn })
      .test();
  ...
  doesAlwaysReturnWithCallback(mockName, funcName, dataToReturn) {
    ...
    preconditions
      ...
      .test();
    preconditions
      .shouldBeArray(dataToReturn, ErrorFactory.build(constants.errorMessages.DoesAlwaysReturnCallbackDataToReturn))
      .debug({ mockName, funcName, dataToReturn })
      .test();
  ...
  doesError(mockName, funcName, dataToReturn) {
    preconditions
      .shouldBeString(mockName, ErrorFactory.build(constants.errorMessages.DoesErrorMockName))
      .debug({ mockName, funcName })
      .test();
    preconditions
      ...
      .test();
    preconditions
      .shouldBeDefined(dataToReturn, ErrorFactory.build(constants.errorMessages.DoesErrorDataToReturn))
      .debug({ mockName, funcName })
      .test();
  ...
  doesErrorWithPromise(mockName, funcName, dataToReturn) {
    preconditions
      .shouldBeString(mockName, ErrorFactory.build(constants.errorMessages.DoesErrorPromiseMockName))
      .debug({ mockName, funcName })
      .test();
    preconditions
      ...
      .test();
    preconditions
      .shouldBeDefined(dataToReturn, ErrorFactory.build(constants.errorMessages.DoesErrorPromiseDataToReturn))
      .debug({ mockName, funcName })
      .test();
  ...
  doesErrorWithCallback(mockName, funcName, dataToReturn) {
    ...
    preconditions
      .shouldBeString(mockName, ErrorFactory.build(constants.errorMessages.DoesErrorCallbackMockName))
      .debug({ mockName, funcName })
      .test();
    preconditions
      ...
      .test();
    preconditions
      .shouldBeArray(dataToReturn, ErrorFactory.build(constants.errorMessages.DoesErrorCallbackDataToReturn))
      .debug({ mockName, funcName, dataToReturn })
      .test();
  ...
  _validate(done) {
    preconditions
      .shouldBeFunction(done, ErrorFactory.build(constants.errorMessages.MissingTestCallback))
      .debug({ done: done })
      .test();
    preconditions
      .shouldBeDefined(this._entryPointObject_, ErrorFactory.build(constants.errorMessages.MissingEntryPoint))
      .debug({ _entryPointObject_: this._entryPointObject_ })
      .test();
  }
  ...
  performance() {
    preconditions
      .shouldBeDefined(this._testContext_, ErrorFactory.build(constants.errorMessages.MissingMochaTestContext))
      .test();
