  ...
  testMock(mockedFunctions, mockName) {
    ...
    _.forEach(mockedFunctions, function (mockedFunction, funcName) {
      let actualByFuncName = mockedFunction.actual,
        ...
        expectedAlwaysByFunName = mockedFunction.expectedAlways;

      // ONLY CHECK LENGTH IF 'expectedAlwaysByFunName' HAS NOT BEEN SET.
      if (!expectedAlwaysByFunName) {
      ...
      for (var i = 0; i < actualByFuncName.length; i++) {
        ...
        for (var paramKey in actualByFuncName[i]) {
          if (actualByFuncName[i].hasOwnProperty(paramKey)) {
            ...
            try {
              actual = actualByFuncName[i][paramKey];
              expected = (expectedAlwaysByFunName && expectedAlwaysByFunName[paramKey]) ?
                expectedAlwaysByFunName[paramKey] : expectedByFuncName[i][paramKey];

              this.tester.shouldEqual(actual, expected, message);
            } catch (err) {
              let throwable = (this._debugFailures_) ? Errr.newError(err.message)
                .debug({actual: actual, expected: expected, alwaysExpected: !!expectedAlwaysByFunName})
