...
import ShouldUseSubset from '../predicates/should-use-subset.js';
...
class Mock {
  ...
  callbackResult(mockName, funcName, mockedData, args) {
    ...
    try {
      ...
      preconditions
        .shouldBeFunction(callback, message)
        .debug({ callbackType: typeof callback })
        .test();
    } catch (err) {
  ...
  mockThisFunction(mockName, funcName, objectToMock) {
    preconditions
      .shouldBeDefined(
        ...
        ErrorFactory.build(constants.errorMessages.FunctionNotInMock, [funcName, mockName])
      )
      .test();
    ...
    preconditions
      .shouldBeUndefined(
        ...
        ErrorFactory.build(constants.errorMessages.MockAlreadyExists, [mockName, funcName])
      )
      .test();

    this._mocks_[mockName][funcName] = Mock.#getNewMockInfo(objectToMock);
    ...
    sinon.stub(objectToMock, funcName).callsFake(
      function () {
        ...
        const args = Mock.#safeClone(argsArr); // clones the arguments so that the mock will have them in the state when they were called regardless if they get modified by the code later on.
      ...
      actualResultsKeys.forEach((actualResultIndex) => {
        ...
        if (mockedFunction.shouldBeCalledUsedAtIndex[actualResultIndex] !== true) {
          ...
          actualResultKeys.forEach((actualParamResultIndex) => {
            ...
            let message = ErrorFactory.build(constants.errorMessages.ComparisonShouldEqual, [
              paramOrdinalValue,
              mockName,
              funcName,
              countOrdinalValue
