...
class Mock {
  ...
  callbackResult(mockName, funcName, mockedData, args) {
    ...
    try {
      preconditions.shouldBeFunction(callback,
        ErrorFactory.build(constants.errorMessages.MissingCallback, [mockName, funcName])).test();
    } catch (err) {
  ...
  doesReturn(mockName, funcName, dataToReturn, returnType, isError) {
    preconditions.shouldBeDefined(this._mocks_[mockName],
      ErrorFactory.build(constants.errorMessages.MissingMockThisFunction, [mockName, funcName])).test();

    preconditions.shouldBeDefined(this._mocks_[mockName][funcName],
      ErrorFactory.build(constants.errorMessages.MissingMockThisFunction, [mockName, funcName])).test();
  ...
  mockThisFunction(mockName, funcName, objectToMock) {
    ...
    sinon.stub(objectToMock, funcName, function () {
      ...
      if (!this.hasTestFinisherBeenExecuted) {
        ...
        if (!mockedData || !mockedData.dataToReturn) {
          const message = ErrorFactory.build(constants.errorMessages.MissingMockedData, [ordinalValues[mock.callCount], mockName, funcName]);
          const error = new Error(message);

          this.setMaddoxRuntimeError(error);
    ...
    _.forEach(mockedFunctions, function (mockedFunction, funcName) {
      ...
      _.forEach(actualResults, function (actualResult, actualResultIndex) {
        ...
        if (numActualParams !== numExpectedParams) {
          const message = ErrorFactory.build(constants.errorMessages.WrongNumberOfParams, [countOrdinalValue, mockName, funcName, numExpectedParams, numActualParams]);
          const error = new Error(message);
