...
class Mock {
  ...
  callbackResult(mockName, funcName, mockedData, args) {
    ...
    try {
      ...
      preconditions.shouldBeFunction(callback, message).test();
    } catch (err) {
    ...
    sinon.stub(objectToMock, funcName).callsFake(function () {
      ...
      if (!this.hasTestFinisherBeenExecuted) {
        ...
        try {
          ...
          preconditions.shouldBeDefined(mockedData, message).test();
          preconditions.shouldBeDefined(mockedData.dataToReturn, message).test();
        } catch (err) {
          this.setMaddoxRuntimeError(err);
