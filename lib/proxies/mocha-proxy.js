...
class Mocha {
    ...
    try {
      ...
      let throwable =
        context.noDebug === true
          ...
          : Errr.newError(err.message)
              .debug({ actual: value, expected: 'Some Truthy Value.' })
              .setAll({ actual: value })
              .appendTo(err)
              .get();
    ...
    try {
      ...
      let throwable =
        context.noDebug === true
          ? Errr.newError(err.message).appendTo(err).setAll({ actual: value }).get()
          : Errr.newError(err.message)
              .debug({ actual: value, expected: 'Some Falsey Value.' })
              .setAll({ actual: value })
              .appendTo(err)
              .get();
  ...
  static subset(actual, expected, message, context) {
    ...
    const isPassing =
      (CoreUtilIs.isString(actual) && CoreUtilIs.isString(expected) && actual.indexOf(expected) !== -1) ||
