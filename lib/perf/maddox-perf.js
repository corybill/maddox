...
class Stats {
  static get(samples) {
    ...
    stats.time.standardError = Math.floor(
      (1000 * (stats.time.standardDeviation / Math.sqrt(stats.totalSampleSize))) / stats.time.mean / 10
    );
    stats.numPerSecond.standardError = Math.floor(
      (1000 * (stats.numPerSecond.standardDeviation / Math.sqrt(stats.totalSampleSize))) / stats.numPerSecond.mean / 10
    );
...
class Samples {
  ...
  static new(context, samplesDone) {
    return new Samples(context, samplesDone);
  }
}
...
class Sample {
  ...
  static new(context, sampleDone) {
    return new Sample(context, sampleDone);
  }
}
