...
var import_preconditions3 = __toESM(require("preconditions"), 1);

// lib/scenarios/scenario.js
var import_preconditions2 = __toESM(require("preconditions"), 1);

// lib/proxies/mocha-proxy.js
var import_variable_diff = __toESM(require("variable-diff"), 1);
var import_chai = __toESM(require("chai"), 1);
var import_errr = __toESM(require("errr"), 1);
var import_util = __toESM(require("util"), 1);
var import_is_subset = __toESM(require("is-subset"), 1);
var import_core_util_is = __toESM(require("core-util-is"), 1);
...
// lib/proxies/mocha-proxy.js
var expect = import_chai.default.expect;
var Mocha = class _Mocha {
  static #shouldEqual(left, right, message) {
    try {
      expect(left, message).eql(right);
    } catch (err) {
      throw import_util.default.isError(left) ? import_errr.default.fromError(left).appendTo(err).throw() : err;
    }
  }
  ...
  static equal(actual, expected, message, context) {
    ...
    try {
      ...
      const diff = (0, import_variable_diff.default)(actual, expected);
      let throwable = context.noDebug === true ? import_errr.default.newError(err.message).setAll({ actual, expected, diff }).appendTo(err).get() : import_errr.default.newError(err.message).debug({ actual, expected }).setAll({ actual, expected, diff }).appendTo(err).get();
  ...
  static truthy(value, message, context) {
    ...
    try {
      ...
      let throwable = context.noDebug === true ? import_errr.default.newError(err.message).appendTo(err).setAll({ actual: value }).get() : import_errr.default.newError(err.message).debug({ actual: value, expected: "Some Truthy Value." }).setAll({ actual: value }).appendTo(err).get();
      throw throwable;
    }
  }
  /**
  ...
  static falsey(value, message, context) {
    ...
    try {
      ...
      let throwable = context.noDebug === true ? import_errr.default.newError(err.message).appendTo(err).setAll({ actual: value }).get() : import_errr.default.newError(err.message).debug({ actual: value, expected: "Some Falsey Value." }).setAll({ actual: value }).appendTo(err).get();
      throw throwable;
    }
  }
  /**
  ...
  static subset(actual, expected, message, context) {
    ...
    context = this.#getContext(context);
    const isPassing = import_core_util_is.default.isString(actual) && import_core_util_is.default.isString(expected) && actual.indexOf(expected) !== -1 || (0, import_is_subset.default)(actual, expected);
    if (!isPassing) {
      let diff = (0, import_variable_diff.default)(actual, expected);
      const split = diff.text.split("\n");
      const startsWithMinus = /\x1b\[[0-9;]+m-/;
      diff.text = split.filter((line) => !startsWithMinus.test(line)).join("\n");
      let throwable = context.noDebug === true ? import_errr.default.newError(message).setAll({ actual, expected, diff }).get() : import_errr.default.newError(message).debug({ actual, expected }).setAll({ actual, expected, diff }).get();
      throwable = this.#appendDiffToTrace(throwable, diff);
      throw throwable;
    }
  }
...
var import_core_util_is2 = __toESM(require("core-util-is"), 1);
var ShouldUseSubset = class {
  static assert(mockedFunction, actualResultIndex, actualParamResult, expectedResultsAlwaysWithSubset) {
    return (mockedFunction.shouldBeCalledWithSubsetUsedAtIndex[actualResultIndex] === true || expectedResultsAlwaysWithSubset === true) && (import_core_util_is2.default.isArray(actualParamResult) || import_core_util_is2.default.isObject(actualParamResult) || import_core_util_is2.default.isString(actualParamResult));
  }
};
...
var ErrorFactory = class {
  static build(errorContext, params) {
    ...
    return import_util2.default.format.apply(this, formatParams);
  }
};
...
var import_preconditions = __toESM(require("preconditions"), 1);
var preconditions = import_preconditions.default.errr();
...
var Mock = class _Mock {
  ...
  mockThisFunction(mockName, funcName, objectToMock) {
    ...
    import_sinon.default.stub(objectToMock, funcName).callsFake(
      function() {
...
var import_simple_statistics = __toESM(require("simple-statistics"), 1);
...
var Stats = class {
  static get(samples) {
    ...
    const allTimesSd = import_simple_statistics.default.standardDeviation(allTimes);
    const allTimesMean = import_simple_statistics.default.mean(allTimes);
    const allNumPerSecondSd = import_simple_statistics.default.standardDeviation(allNumPerSecond);
    const allNumPerSecondMean = import_simple_statistics.default.mean(allNumPerSecond);
    ...
    const stats = {
      ...
      time: {
        mean: import_simple_statistics.default.mean(allTimes),
        median: import_simple_statistics.default.median(allTimes),
        standardDeviation: import_simple_statistics.default.standardDeviation(allTimes),
        variance: import_simple_statistics.default.variance(allTimes),
        dropped: droppedAllTimesCount
      },
      numPerSecond: {
        mean: import_simple_statistics.default.mean(allNumPerSecond),
        median: import_simple_statistics.default.median(allNumPerSecond),
        standardDeviation: import_simple_statistics.default.standardDeviation(allNumPerSecond),
        variance: import_simple_statistics.default.variance(allNumPerSecond),
...
var import_errr2 = __toESM(require("errr"), 1);
var HandleError = class {
  static next(state, err) {
    ...
    if (hasTestableBeenExecuted) {
      import_errr2.default.newError(error_factory_default.build(constants_default.errorMessages.TestFailure)).appendTo(err).throw();
    } else if (maddoxRuntimeError) {
      const errr = import_errr2.default.fromError(maddoxRuntimeError);
      if (executionError) {
        errr.appendTo(executionError);
      }
      errr.throw();
    } else {
      const errr = import_errr2.default.fromError(err);
      if (executionError) {
        errr.appendTo(executionError);
      }
      testable(errr.get());
...
// lib/scenarios/http-req-scenario.js
var preconditions3 = import_preconditions3.default.errr();
...
var import_preconditions4 = __toESM(require("preconditions"), 1);
var preconditions4 = import_preconditions4.default.errr();
...
var import_preconditions5 = __toESM(require("preconditions"), 1);
var preconditions5 = import_preconditions5.default.errr();
...
var import_preconditions6 = __toESM(require("preconditions"), 1);
var preconditions6 = import_preconditions6.default.errr();
...
var FrameworkRouteScenario = class extends scenario_default {
  ...
  addStub(descriptor) {
    ...
    const module2 = descriptor.module;
